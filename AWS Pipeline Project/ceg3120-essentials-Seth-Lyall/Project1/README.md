...
5. Creating a **Security Group**
   - A security group is a stateful firewall that determines which IPs are allowed to connect to the given VPC.
     ...
     - Under `Inbound rules` enter the following rules:
       - Type: `SSH`, Source: `Custom` `174.97.0.0/16`, Description: `Home IP allowed to ssh`
       ...
       - Type: `Custom ICMP - IPv4`, Source: `Custom` `174.97.0.0/16`, Description: `Home IP allowed to request ICMP`
...
3. Testing the **Security**
   ...
   ![A successful ssh from wright state](images/wright_state_ssh.png)
     - A successful `ssh` from `174.97.0.0/16` (my home ip): `ssh -i (location of private key file) ubuntu@(instance public IPv4 address)`
